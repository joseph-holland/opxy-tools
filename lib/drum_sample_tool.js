    ...
    advancedBtn.addEventListener('click', function() {
        ...
        zoomModalElement.addEventListener('shown.bs.modal', () => {
            ...
            function drawZoomWaveform() {
                ...
                // Draw min-max envelopes (corrected orientation)
                ctx.fillStyle = '#333';
                for (let x = 0; x < width; x++) {
                    ...
                    // CORRECTED: positive values go up, negative go down
                    const minY = amp - min * amp;
                    const maxY = amp - max * amp;
                    ctx.fillRect(x, maxY, 1, minY - maxY);
                }
            }
        ...
        function drawMarkers() {
            ...
            // Draw min-max envelopes for better waveform visualization
            ctx.fillStyle = '#333';
            for (let x = 0; x < width; x++) {
                const start = Math.floor(x * step);
                const end = Math.min(Math.floor((x + 1) * step), data.length);
