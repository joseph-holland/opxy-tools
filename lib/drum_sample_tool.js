...
function playSample(idx) {
    ...
    // === Gain ===
    let gainValue = 0;
    if (advancedBtn && typeof advancedBtn.dataset.gain !== 'undefined' && advancedBtn.dataset.gain !== null) {
...
for (let i = 1; i <= NUM_DRUMS; i++) {
    ...
    advancedBtn.addEventListener('click', function() {
        ...
        if (modal) {
            ...
            modal.querySelector('.adv-reverse-label').textContent = tempSettings.isReverse ? 'reverse' : 'forward';
        ...
        zoomModalElement.addEventListener('shown.bs.modal', () => {
            ...
            function findNearestZeroCrossing(position, searchDirection, maxSearchDistance = 1000) {
                ...
                if (searchDirection > 0) {
                    ...
                    for (let i = position; i < position + maxSearch && i < data.length - 1; i++) {
            ...
            function updateSampleValueDisplays() {
                ...
                if (startValueElement && startFrame >= 0 && startFrame < data.length) {
            ...
            canvas.addEventListener('mousemove', (e) => {
                ...
                if (dragging === 'start') {
                    ...
                    if (zeroSnap) {
                        // Find nearest positive-going zero crossing
                        const zeroCrossingPos = findNearestZeroCrossing(frame, -1, 500);
                        endFrame = Math.max(startFrame + 1, Math.min(zeroCrossingPos, data.length));
                    } else {
        ...
        while (waveformDiv.firstChild) waveformDiv.removeChild(waveformDiv.firstChild);
        ...
        startInput.oninput = endInput.oninput = function() {
            ...
            let s = Math.max(0, Math.min(parseInt(startInput.value, 10) || 0, buffer.length-1));
            let e = Math.max(s+MIN_DISTANCE, Math.min(parseInt(endInput.value, 10) || buffer.length, buffer.length));
            startInput.value = s;
            endInput.value = e;
            }
            clampMarkers();
        ...
        canvas.addEventListener('mousedown', function(e) {
            ...
            if (Math.abs(x - sx) < Math.abs(x - ex)) {
                startInput.value = Math.max(0, Math.min(Math.round((x / width) * buffer.length), buffer.length-1));
                tempSettings.sampleStart = parseInt(startInput.value, 10);
            } else {
                endInput.value = Math.max(parseInt(startInput.value,10)+10, Math.min(Math.round((x / width) * buffer.length), buffer.length));
                tempSettings.sampleEnd = parseInt(endInput.value, 10);
            }
            drawMarkers();
            if (playTimeout) clearTimeout(playTimeout);
            ...
            dragMoveHandler = function(e) {
                ...
                if (draggingMarker === 'start') {
                    ...
                    endInput.value = Math.max(startFrame + 1, Math.min(frame, buffer.length));
                    tempSettings.sampleEnd = parseInt(endInput.value, 10);
                }
                drawMarkers();
                if (playTimeout) clearTimeout(playTimeout);
...
const drumGenerateBtn = Array.from(document.querySelectorAll('#drum .accent-btn')).find(btn => btn.textContent.toLowerCase().includes('generate patch'));
if (drumGenerateBtn) {
        ...
        try {
            ...
            if (window.advancedPresetSettings) {
                ...
                if (patchJson.engine) {
                    ...
                    if (!isNaN(adv.velocity)) patchJson.engine['velocity.sensitivity'] = adv.velocity;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  syncPercentageControls('preset-velocity-sensitivity', 'preset-velocity-sensitivity-number');
  ...
  const initializeAdvancedPresetDefaults = () => {
    ...
    const velocitySlider = document.getElementById('preset-velocity-sensitivity');
  ...
  const savePresetBtn = document.getElementById('save-preset-advanced-settings');
  if (savePresetBtn) {
    savePresetBtn.addEventListener('click', function() {
      const playmode = document.getElementById('preset-playmode').value;
      const transpose = parseInt(document.getElementById('preset-transpose').value, 10);
      const velocityPercent = parseInt(document.getElementById('preset-velocity-sensitivity').value, 10);
      const volumePercent = parseInt(document.getElementById('preset-volume').value, 10);
      const widthPercent = parseInt(document.getElementById('preset-width').value, 10);
      ...
      window.advancedPresetSettings = { playmode, transpose, velocity, volume, width };
      
      // Hide modal
      const modal = bootstrap.Modal.getInstance(document.getElementById('preset-advanced-modal'));
      if (modal) modal.hide();
    });
  }
});
...
// Handle collapse chevron rotation for drum sections
function initDrumCollapseSections() {
  const drumHowToUseCollapse = document.getElementById('drumHowToUseCollapse');
  const drumSettingsCollapse = document.getElementById('drumSettingsCollapse');
  const drumHowToUseChevron = document.getElementById('drumHowToUseChevron');
  const drumSettingsChevron = document.getElementById('drumSettingsChevron');
  
  if (drumHowToUseCollapse && drumHowToUseChevron) {
    drumHowToUseCollapse.addEventListener('show.bs.collapse', function() {
      drumHowToUseChevron.style.transform = 'rotate(0deg)';
    });
    drumHowToUseCollapse.addEventListener('hide.bs.collapse', function() {
      drumHowToUseChevron.style.transform = 'rotate(-90deg)';
    });
  }
  
  if (drumSettingsCollapse && drumSettingsChevron) {
    drumSettingsCollapse.addEventListener('show.bs.collapse', function() {
      drumSettingsChevron.style.transform = 'rotate(0deg)';
    });
    drumSettingsCollapse.addEventListener('hide.bs.collapse', function() {
      drumSettingsChevron.style.transform = 'rotate(-90deg)';
    });
  }
}
...
window.addEventListener('resize', function() {
  ...
  if (modal && modal.classList.contains('show')) {
    ...
    const buffer = window.drumSamples && window.drumSamples[modal.dataset.sampleIndex];
  ...
  if (zoomModal && zoomModal.classList.contains('show')) {
    ...
    const buffer = window.drumSamples && window.drumSamples[zoomModal.dataset.sampleIndex];
    if (canvas && buffer) {
      ...
      drawBuffer(width, height, ctx, buffer);
      // Redraw markers if needed
      if (typeof window.drawZoomMarkers === 'function') window.drawZoomMarkers();
    }
  }
});
...
function updateGeneratePatchBtnState() {
  const btn = Array.from(document.querySelectorAll('#drum .accent-btn')).find(btn => btn.textContent.toLowerCase().includes('generate patch'));
  if (btn) {
...
function updateDrumPatchSizeIndicator() {
  ...
  if (statusElement) {
    ...
    if (percentage > 90) {
      ...
      statusElement.textContent = 'patch size too large!';
    } else if (percentage > 70) {
...
function updateDrumPatchSize() {
  ...
  for (let i = 1; i <= NUM_DRUMS; i++) {
    if (drumSamples[i]) {
      drumTotalPatchSize += calculateSampleSize(drumSamples[i]);
    }
  }
