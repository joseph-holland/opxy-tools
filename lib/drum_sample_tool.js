        ...
        canvas.addEventListener("mousedown", (e) => {
          ...
          if (candidateMarkers.length > 0) {
            ...
            if (y >= bottomZone || candidateMarkers.length === 1) {
              dragging = candidateMarkers.reduce((a, b) => 
            ...
            if (nearestMarker === "start") {
              let newStart;
              if (zeroSnap) {
                newStart = findNearestZeroCrossing(frame, 1, 500);
              } else {
                newStart = frame;
              }
              ...
              if (newStart >= endFrame) {
                endFrame = Math.min(newStart + 1, data.length);
                endInput.value = endFrame;
              }
            } else {
              let newEnd;
              if (zeroSnap) {
                newEnd = findNearestZeroCrossing(frame, -1, 500);
              } else {
                newEnd = frame;
              }
              
              newEnd = Math.max(startFrame + 1, Math.min(newEnd, data.length));
              endFrame = newEnd;
              endInput.value = endFrame;
            }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (bitDepthSelect) {
    bitDepthSelect.addEventListener("change", function () {
      // Update patch size when bit depth changes
      updateDrumPatchSize();
    });
  }
});
...
function calculateSampleSize(audioBuffer, bitDepth = "keep") {
  ...
  // Estimate WAV file size: samples * channels * bytes per sample + header
  const samples = audioBuffer.length;
  const channels = audioBuffer.numberOfChannels;
  return samples * channels * bytesPerSample + 44; // 44 bytes for WAV header
}
...
function updateDrumPatchSize() {
  ...
  // Get the selected sample rate and bit depth for size calculation
  ...
  for (let i = 1; i <= NUM_DRUMS; i++) {
    if (drumSamples[i]) {
      ...
      if (targetSampleRate === 0) {
        // Keep original - use original buffer size with selected bit depth
        sampleSize = calculateSampleSize(drumSamples[i], targetBitDepth);
      } else {
        // Re-encode - calculate size based on target sample rate and bit depth
        ...
        sampleSize = calculateSampleSize(mockBuffer, targetBitDepth);
      }
...
function drawDrumRecordingWaveform() {
  ...
  // Background - consistent with multisample tool
  ctx.fillStyle = '#ececec';
  ctx.fillRect(0, 0, width, height);

  // Draw waveform - consistent with multisample tool
  ctx.fillStyle = '#333';
  const step = Math.ceil(data.length / width);
  const amp = height / 2;
  ...
  // Draw center line - consistent with multisample tool
  ctx.strokeStyle = '#333';
  ctx.lineWidth = 1;
  ctx.beginPath();
...
// === Drum Import Settings Functionality ===
function initializeDrumImportSettings() {
  // Drum import settings
  const drumImportBtn = document.getElementById('import-drum-settings-btn');
  const drumImportInput = document.getElementById('import-drum-settings-input');
  
  if (drumImportBtn && drumImportInput) {
    drumImportBtn.addEventListener('click', () => {
      drumImportInput.click();
    });
    
    drumImportInput.addEventListener('change', handleDrumImport);
  }
}

async function handleDrumImport(event) {
  ...
  try {
    ...
    // Validate that this is a drum preset
    if (json.type !== 'drum') {
      throw new Error('this is not a drum preset. please select a drum preset file.');
    }
    
    // Import engine settings
    importDrumEngineSettings(json.engine);
    
    // Update UI to reflect imported settings
    updateDrumAdvancedUI();
    
    // Show success message
    showDrumImportSuccess('Drum engine settings imported successfully!');
    
  } catch (error) {
    showDrumImportError('Failed to import settings: ' + error.message);
  } finally {
...
function importDrumEngineSettings(engineSettings) {
  // Create or update the global drum advanced settings
  if (!window.advancedPresetSettings) {
    window.advancedPresetSettings = {};
  }
  
  if (engineSettings.playmode !== undefined) {
    window.advancedPresetSettings.playmode = engineSettings.playmode;
  }
  
  if (engineSettings.transpose !== undefined) {
    window.advancedPresetSettings.transpose = engineSettings.transpose;
  }
  
  if (engineSettings.velocity !== undefined) {
    window.advancedPresetSettings.velocity = engineSettings.velocity;
  }
  
  if (engineSettings.volume !== undefined) {
    window.advancedPresetSettings.volume = engineSettings.volume;
  }
  
  if (engineSettings.width !== undefined) {
    window.advancedPresetSettings.width = engineSettings.width;
  }
}

function updateDrumAdvancedUI() {
  if (!window.advancedPresetSettings) return;
  
  const settings = window.advancedPresetSettings;
  ...
  const transposeSlider = document.getElementById('preset-transpose');
  const transposeNumber = document.getElementById('preset-transpose-number');
  if (transposeSlider && transposeNumber && settings.transpose !== undefined) {
    transposeSlider.value = settings.transpose;
  ...
  if (velocitySlider && velocityNumber && settings.velocity !== undefined) {
    const velocityPercent = Math.round((settings.velocity / 32767) * 100);
  ...
  const volumeSlider = document.getElementById('preset-volume');
  const volumeNumber = document.getElementById('preset-volume-number');
  if (volumeSlider && volumeNumber && settings.volume !== undefined) {
  ...
  const widthSlider = document.getElementById('preset-width');
  const widthNumber = document.getElementById('preset-width-number');
  if (widthSlider && widthNumber && settings.width !== undefined) {
...
function showDrumImportSuccess(message) {
  // Create a temporary success message
  const alertDiv = document.createElement('div');
  alertDiv.className = 'alert alert-dismissible fade show position-fixed';
  alertDiv.style.cssText = 'top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999; max-width: 500px; text-align: center; background-color: #f8f9fa; border: 1px solid #868686; color: #333;';
  alertDiv.innerHTML = `
    ${message.toLowerCase()}
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="close" style="filter: none; background: none; border: none; font-size: 1.2em; color: #333; opacity: 0.7;">&times;</button>
  `;
...
function showDrumImportError(message) {
  // Create a temporary error message
  const alertDiv = document.createElement('div');
  alertDiv.className = 'alert alert-dismissible fade show position-fixed';
  alertDiv.style.cssText = 'top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999; max-width: 500px; text-align: center; background-color: #f8f9fa; border: 1px solid #333; color: #333;';
  alertDiv.innerHTML = `
    ${message.toLowerCase()}
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="close" style="filter: none; background: none; border: none; font-size: 1.2em; color: #333; opacity: 0.7;">&times;</button>
  `;
