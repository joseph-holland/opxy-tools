...
document.addEventListener('DOMContentLoaded', function() {
  ...
  syncSliderAndNumber('preset-velocity-sensitivity', 'preset-velocity-sensitivity-number', 0, 32767);
  syncSliderAndNumber('preset-volume', 'preset-volume-number', 0, 32767);
  syncSliderAndNumber('preset-width', 'preset-width-number', 0, 32767);
  ...
  const initializeAdvancedPresetDefaults = () => {
    ...
    if (velocitySlider) velocitySlider.value = 19660;
    if (velocityNumber) velocityNumber.value = 19660;
    if (volumeSlider) volumeSlider.value = 18348;
    if (volumeNumber) volumeNumber.value = 18348;
    if (widthSlider) widthSlider.value = 0;
    if (widthNumber) widthNumber.value = 0;

    // Initialize the global settings object with defaults
  ...
  if (savePresetBtn) {
    savePresetBtn.addEventListener('click', function() {
      ...
      const velocity = parseInt(document.getElementById('preset-velocity-sensitivity').value, 10);
      const volume = parseInt(document.getElementById('preset-volume').value, 10);
