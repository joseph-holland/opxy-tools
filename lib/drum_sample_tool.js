    ...
    advancedBtn.addEventListener("click", function () {
      ...
      if (modal) {
        ...
        modal.querySelector(".adv-play-modal").onclick = () => {
          const startFrame = parseInt(modal.querySelector(".adv-sample-start").value) || 0;
          const endFrame = parseInt(modal.querySelector(".adv-sample-end").value) || drumSamples[sampleIndex].audioBuffer.length;
          const panValue = parseInt(modal.querySelector("#advancedPanNumber").value) || 0;
          playSampleSlice(sampleIndex, startFrame, endFrame, panValue);
        };
        ...
        newCanvas.addEventListener("mousemove", function (e) {
          ...
          if (dragging) {
            ...
            if (dragging === "start") {
              ...
              startFrame = newStart;
              startInput.value = newStart;
              tempSettings.sampleStart = newStart; // Update tempSettings immediately
              
              // Push end marker if start goes past it
              if (newStart >= endFrame) {
                endFrame = Math.min(newStart + 10, data.length);
                endInput.value = endFrame;
                tempSettings.sampleEnd = endFrame; // Update tempSettings immediately
              }
              ...
              endFrame = newEnd;
              endInput.value = endFrame;
              tempSettings.sampleEnd = newEnd; // Update tempSettings immediately
            }
