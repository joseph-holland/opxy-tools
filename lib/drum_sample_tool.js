...
for (let i = 1; i <= NUM_DRUMS; i++) {
    ...
    advancedBtn.addEventListener('click', function() {
        ...
        if (modal) {
            ...
            modal.querySelector('.adv-reverse-label').textContent = tempSettings.isReverse ? 'Reverse' : 'Normal';
        ...
        zoomModalElement.addEventListener('shown.bs.modal', () => {
            ...
            canvas.addEventListener('mousedown', (e) => {
                ...
                // Increase threshold for easier grabbing
                const grabThreshold = 12;

                if (Math.abs(x - startX) < grabThreshold) {
                    dragging = 'start';
                } else if (Math.abs(x - endX) < grabThreshold) {
                    dragging = 'end';
                }
            });
            ...
            document.getElementById('zoom-save-button').onclick = () => {
                ...
                if (advancedModal) {
                    ...
                    if (typeof advancedModal.drawMarkers === 'function') {
                        ...
                        // Fallback: try to find and call drawMarkers from closure
