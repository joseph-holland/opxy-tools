...
function updateSampleUI(sampleIndex) {
  ...
  let bitDepthText = `<i class="fa fa-database"></i>${sample.originalBitDepth}-bit`;
  if (effectiveBitDepth !== sample.originalBitDepth) {
    bitDepthText += `<i class="fa fa-arrow-right"></i>${effectiveBitDepth}-bit`;
  }
  
  let sampleRateText = `<i class="fa fa-tachometer-alt"></i>${(sample.originalSampleRate / 1000).toFixed(1)}k`;
  if (effectiveSampleRate !== sample.originalSampleRate) {
    sampleRateText += `<i class="fa fa-arrow-right"></i>${(effectiveSampleRate / 1000).toFixed(1)}k`;
  }
  
  // Channel info with icons
  let channelIcon = sample.originalChannels === 1 ? 'fa-volume-down' : 'fa-volume-up';
  let channelText = `<i class="fa ${channelIcon}"></i>${sample.originalChannels === 1 ? 'M' : 'S'}`;
  if (effectiveChannels !== sample.originalChannels) {
    const newIcon = effectiveChannels === 1 ? 'fa-volume-down' : 'fa-volume-up';
    const newText = effectiveChannels === 1 ? 'M' : 'S';
    channelText += `<i class="fa fa-arrow-right"></i><i class="fa ${newIcon}"></i>${newText}`;
  }
  
  // Duration and file size
  const durationText = `<i class="fa fa-clock"></i>${sample.duration < 1 ? 
    `${Math.round(sample.duration * 1000)}ms` : 
    `${sample.duration.toFixed(1)}s`}`;
  
  const fileSizeKB = (sample.fileSize / 1024).toFixed(1);
  const fileSizeText = `<i class="fa fa-hdd"></i>${fileSizeKB}k`;
