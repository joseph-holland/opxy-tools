  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const sampleRate = dataView.getUint32(24, true);  // Sample rate is at offset 24
        const bitDepth = dataView.getUint16(34, true);    // Bits per sample at offset 34

        // Parse basic WAV header info for other properties
        const audioContext = new AudioContext();
...
function audioBufferToWavWithBitDepth(audioBuffer, requestedBitDepth) {
  ...
  let finalDepth = requestedBitDepth;
  if (finalDepth === "keep") {
    finalDepth = 16;
  }
...
function encodeAudioBufferAsWavBlob(audioBuffer, bitDepth) {
  ...
  for (let i = 0; i < length; i++) {
    for (let ch = 0; ch < numChannels; ch++) {
      let sampleFloat = audioBuffer.getChannelData(ch)[i];
      sampleFloat = Math.max(-1, Math.min(1, sampleFloat));

      if (bitDepth === 16) {
        const intSample = sampleFloat < 0 ? sampleFloat * 0x8000 : sampleFloat * 0x7fff;
        writeInt16LE(view, offset, intSample);
        offset += 2;
      } else {
        // 24-bit
        let intSample24 = sampleFloat < 0 ? sampleFloat * 0x800000 : sampleFloat * 0x7fffff;
        intSample24 = Math.floor(intSample24);
        writeInt24LE(view, offset, intSample24);
        offset += 3;
      }
    }
  }
