...
function updateMultiPatchSizeIndicator() {
  ...
  if (percentage >= 90) {
    statusElement.textContent = 'Approaching limit - consider reducing samples';
    progressBar.className = 'progress-bar danger';
  } else if (percentage >= 70) {
    statusElement.textContent = 'Good usage';
...
function updateMultiPatchSize() {
  ...
  multisampleFiles.forEach(sample => {
    if (sample && sample.audioBuffer) {
      multiTotalPatchSize += calculateSampleSize(sample.audioBuffer);
    }
  });
  ...
  if (multiBrowseBtn && multiFileInput) {
    ...
    multiFileInput.addEventListener('change', function(e) {
      if (multiFileInput.files && multiFileInput.files.length > 0 && window.processMultisampleFiles) {
        processMultisampleFiles(Array.from(multiFileInput.files));
      }
    });
  }
}

// Handle collapse chevron rotation for multisample sections
function initMultisampleCollapseSections() {
  const howToUseCollapse = document.getElementById('howToUseCollapse');
  const sampleReqCollapse = document.getElementById('sampleReqCollapse');
  const settingsCollapse = document.getElementById('settingsCollapse');
  const howToUseChevron = document.getElementById('howToUseChevron');
  const sampleReqChevron = document.getElementById('sampleReqChevron');
  const settingsChevron = document.getElementById('settingsChevron');
  
  if (howToUseCollapse && howToUseChevron) {
    howToUseCollapse.addEventListener('show.bs.collapse', function() {
      howToUseChevron.style.transform = 'rotate(0deg)';
    });
    howToUseCollapse.addEventListener('hide.bs.collapse', function() {
      howToUseChevron.style.transform = 'rotate(-90deg)';
    });
  }
  
  if (sampleReqCollapse && sampleReqChevron) {
    sampleReqCollapse.addEventListener('show.bs.collapse', function() {
      sampleReqChevron.style.transform = 'rotate(0deg)';
    });
    sampleReqCollapse.addEventListener('hide.bs.collapse', function() {
      sampleReqChevron.style.transform = 'rotate(-90deg)';
    });
  }
  
  if (settingsCollapse && settingsChevron) {
    settingsCollapse.addEventListener('show.bs.collapse', function() {
      settingsChevron.style.transform = 'rotate(0deg)';
    });
    settingsCollapse.addEventListener('hide.bs.collapse', function() {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initMultisampleCollapseSections();
});
