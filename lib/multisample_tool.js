...
function drawMultisampleMarkers(canvas, sampleData) {
  ...
  // In/Out marker handles (circles at bottom) - dark grey for monochrome
    ...
    if (sampleData) {
      ...
      if (draggingMarker === "inPoint") {
        const newInPoint = Math.max(
          0,
          Math.min(newTime, sampleData.outPoint - 0.01)
        );
        ...
        sampleData.loopStart = Math.max(
          sampleData.inPoint,
          Math.min(newTime, sampleData.loopEnd - 0.01)
        );
      } else if (draggingMarker === "loopEnd") {
        sampleData.loopEnd = Math.max(
          sampleData.loopStart + 0.01,
          Math.min(newTime, sampleData.outPoint)
        );
      } else if (draggingMarker === "outPoint") {
        const newOutPoint = Math.max(
          sampleData.inPoint + 0.01,
          Math.min(newTime, sampleData.duration)
        );
...
function setupMultisampleZoomModal() {
  ...
  modalElement.addEventListener('shown.bs.modal', () => {
    ...
    let inFrame = Math.floor((sampleData.inPoint / sampleData.duration) * buffer.length);
    let loopStartFrame = Math.floor((sampleData.loopStart / sampleData.duration) * buffer.length);
    let loopEndFrame = Math.floor((sampleData.loopEnd / sampleData.duration) * buffer.length);
    let outFrame = Math.floor((sampleData.outPoint / sampleData.duration) * buffer.length);
    ...
    function drawMultisampleZoomMarkers() {
      ...
      // In/Out marker handles
      ctx.fillStyle = '#333';
      ctx.beginPath();
      ctx.arc(inPos, height - 10, 8, 0, 2 * Math.PI);
      ctx.arc(outPos, height - 10, 8, 0, 2 * Math.PI);
      ctx.fill();

      // Loop markers (light grey)
      const triangleSize = 10;
      ctx.fillStyle = '#666';
      ctx.strokeStyle = '#666';
      ctx.lineWidth = 2;

      // Loop start triangle and line
      ctx.beginPath();
      ctx.moveTo(loopStartPos - triangleSize / 2, 0);
      ctx.lineTo(loopStartPos + triangleSize / 2, 0);
      ctx.lineTo(loopStartPos, triangleSize);
    ...
    canvas.addEventListener('mousedown', (e) => {
      ...
      const distances = {
        in: Math.abs(x - inPos),
        loopStart: Math.abs(x - loopStartPos),
        loopEnd: Math.abs(x - loopEndPos),
        out: Math.abs(x - outPos)
      };
      ...
      if (distances[closest] < 10) {
        ...
        switch (dragging) {
          case 'in':
            inFrame = Math.max(0, Math.min(targetFrame, loopStartFrame - 1));
            inInput.value = inFrame;
            break;
          case 'loopStart':
          ...
          case 'loopEnd':
            ...
            break;
          case 'out':
            outFrame = Math.max(loopEndFrame, Math.min(targetFrame, data.length));
            outInput.value = outFrame;
            break;
        }
        updateMultisampleZoomWaveformAndMarkers();
